    ...
    def train(self, data:np.ndarray, data_A:np.ndarray, iter_all: int, is_train: bool = True, is_initial_local: bool=True):
        ...
        for iter in range(self._model_setting.Iteration):
            ...
            for t in range(self._model_setting.T):  # from layer 1 to T
                ...
                # Xt1 = self.sampler.crt(Xt_to_t1[t-1].astype('double'), np.dot(self.global_params.Phi[t], self.local_params.Theta[t]))
                # Xt_to_t1[t], WSZS[t] = self.sampler.multi_aug(Xt1.astype('double'), self.global_params.Phi[t], self.local_params.Theta[t])
            ...
            #                                                  / (self.local_params.Theta[t - 1][:, n].sum(0) + self._hyper_params.c_j_f0)
            #     p_j_tmp = self.sampler._calculate_pj(self.local_params.c_j, self._model_setting.T)
            ...
            LH = np.sum(data * log_max(re) - re) #- log_max(gamma(dataset+1)))
