...
class WEDTM(Basic_Model):
    ...
    def initial(self, data):
        ...
        Inputs:
            dataset : [np.ndarray] or [scipy.sparse.csc.csc_matrix] V*N matrix, N bag-of-words vectors with a vocabulary length of V
            ...
            for iter in range(1, self._model_setting.Iteration[Tcurrent]):
                ...
                for t in range(Tcurrent + 1):
                    if t == 0:
                        ...
                        beta1 = self._sample_beta(WSZS[t].T, embeddings, beta1)
                    else:
    ...
    def _sample_rk(self,XTplusOne_sum=None, r_k=None, p_jTplusOne=None, gamma0=None, c0=None, IsNoSample=None, e0=None,
        ...
        if ~IsNoSample:
            ...
            # XTplusOne_sum
            gamma0 = self._sampler.gamma(a0 + self._crt_sum_mex_v1(XTplusOne_sum, (gamma0 / KT))) / (b0 - np.log(max(1 - p_prime, 2.2251e-308)))
