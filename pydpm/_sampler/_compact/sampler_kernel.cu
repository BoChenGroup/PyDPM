...
#include "sampler_kernel.h"
// 本版本为kernel的简化版本，保留init，gamma等功能，同时将status结构体直接更换为随机数发生器，增加随机数发生器规模接口并优化其使用情况

// ------------------------------------------------substorage ------------------------------------------
DLLEXPORT size_t _get_nElems(void* sst) {
    return ((substorage*)sst)->nElems;
}

DLLEXPORT void* _init_substorage(size_t nElems) {
...
DLLEXPORT void _free_substorage(void* sst) {
...
__global__ void _rand_noncentral_chisquare(int* degrees, float* loc, float* scale, float* output, size_t matrix_scale, size_t nElems, curandStateXORWOW_t* status) {
    size_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx < nElems) {
        size_t matrix_idx = idx % matrix_scale;
        size_t state_idx = idx % nStatus;
        int N = degrees[matrix_idx];
        float mean = sqrtf(loc[matrix_idx] / N);
        float Var = scale[matrix_idx];
        float sum = 0;
        for (int i = 0; i < N; i++) {
            float x = powf(curand_normal(&status[state_idx])*Var + mean, 2);
            sum += x;
        }
        output[idx] = sum;
    }
}
DLLEXPORT void _sample_noncentral_chisquare(int* degrees_host, float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    int* degrees_device;
    size_t nBytes = matrix_scale * sizeof(int);
    cudaMalloc((void**)&degrees_device, nBytes);
    cudaMemcpy(degrees_device, degrees_host, nBytes, cudaMemcpyHostToDevice);
    ...
    dim3 grid(size_t(ceil(float(nElems) / float(nThreads_gamma)))), block(nThreads_gamma);
    _rand_noncentral_chisquare <<<grid, block>>> (degrees_device, loc_device, scale_device, output_device, matrix_scale, nElems, status);
    cudaMemcpy(output, output_device, nBytes, cudaMemcpyDeviceToHost);
...
// -------------------------------------------------sample noncentral_f ---------------------------------------------
__global__ void _rand_noncentral_f(int* n1, int* n2, float* loc, float* scale, float* output, size_t matrix_scale, size_t nElems, curandStateXORWOW_t* status) {
    size_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx < nElems) {
        size_t matrix_idx = idx % matrix_scale;
        size_t state_idx = idx % nStatus;
        int N1 = n1[matrix_idx];
        int N2 = n2[matrix_idx];
        float mean = sqrtf(loc[matrix_idx] / N1);

        float sum1 = 0;
        float sum2 = 0;
        for (int i = 0; i < N1; i++) {
            float x = powf(curand_normal(&status[state_idx])*scale[matrix_idx] + mean, 2);
            sum1 += x;
        }
        for (int i = 0; i < N2; i++) {
            float x = powf(curand_normal(&status[state_idx]),2);
            sum2 += x;
        }

        output[idx] = (sum1 / N1) / (sum2 / N2);
    }
}
DLLEXPORT void _sample_noncentral_f(int* n1_host, int* n2_host, float* loc_host, float* scale_host, float* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    int* n1_device, * n2_device;
    size_t nBytes = matrix_scale * sizeof(int);
    cudaMalloc((void**)&n1_device, nBytes);
    cudaMalloc((void**)&n2_device, nBytes);
    ...
    dim3 grid(size_t(ceil(float(nElems) / float(nThreads_gamma)))), block(nThreads_gamma);
    _rand_noncentral_f <<<grid, block>>> (n1_device, n2_device, loc_device, scale_device, output_device, matrix_scale, nElems, status);
    cudaMemcpy(output, output_device, nBytes, cudaMemcpyDeviceToHost);
...
__global__ void _rand_hypergeometric(float* prob, int* nsample, int* output, size_t matrix_scale, size_t nElems, curandStateXORWOW_t* status) {
    size_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx < nElems) {
        size_t matrix_idx = idx % matrix_scale;
        size_t state_idx = idx % nStatus;
        float p = prob[matrix_idx];
        int n = nsample[matrix_idx];
        int rnd = 0;
        for (int i = 0; i < n; i++) {
            if (curand_uniform(&status[state_idx]) < p)
                rnd++;
        }
        output[idx] = rnd;
    }
}
DLLEXPORT void _sample_hypergeometric(float* prob_host, int* nsample_host, int* output, size_t matrix_scale, size_t repeats, curandStateXORWOW_t* status) {
    float* prob_device;
    size_t nBytes = matrix_scale * sizeof(float);
    cudaMalloc((void**)&prob_device, nBytes);
    cudaMemcpy(prob_device, prob_host, nBytes, cudaMemcpyHostToDevice);
    int* nsample_device;
    nBytes = matrix_scale * sizeof(int);
    ...
    dim3 grid(size_t(ceil(float(nElems) / float(nThreads_gamma)))), block(nThreads_gamma);
    _rand_hypergeometric <<<grid, block>>> (prob_device, nsample_device, output_device, matrix_scale, nElems, status);
    cudaMemcpy(output, output_device, nBytes, cudaMemcpyDeviceToHost);
...
__global__ void _rand_multinomial(int* count, float* prob, int* output, size_t matrix_scale_1, size_t matrix_scale_2, size_t repeats, curandStateXORWOW_t* status) {
    ...
    if (idx < matrix_scale_1*repeats) {
        size_t K = int(float(matrix_scale_2) / float(matrix_scale_1));
        size_t matrix_idx = idx % matrix_scale_1;  // row idx in prob_device
        size_t repeat_idx = floor(float(idx) / float(matrix_scale_1));  // times
        size_t state_idx = idx % nStatus;
